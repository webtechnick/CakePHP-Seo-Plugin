...
class SeoUrisController extends SeoAppController {
	...
	function admin_urlencode($id = null){
		if($this->SeoUri->urlEncode($id)){
			$this->goodFlash("uri Successfully Url Encoded.");
		}
		else {
			$this->badFlash("Erorr URL Encoding uri");
		}
	...
	function admin_view($id = null) {
		if (!$id) {
			$this->badFlash(__('Invalid seo uri'));
	...
	function admin_add() {
		if (!empty($this->request->data)) {
			...
			if ($this->SeoUri->saveAll($this->request->data)) {
				$this->goodFlash(__('The seo uri has been saved'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->badFlash(__('The seo uri could not be saved. Please, try again.'));
			}
		}
	}
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->badFlash(__('Invalid id for seo uri'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->SeoUri->delete($id)) {
			$this->goodFlash(__('Seo uri deleted'));
			$this->redirect(array('action'=>'index'));
		}
		$this->badFlash(__('Seo uri was not deleted'));
		$this->redirect(array('action' => 'index'));
	}
