...
class SeoStatusCodesController extends SeoAppController {
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->SeoStatusCode->save($this->data)) {
				$this->Session->setFlash(__('The seo status code has been saved', true));
				$this->redirect(array('action' => 'index'));
			} else {
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for seo status code', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->SeoStatusCode->delete($id)) {
			$this->Session->setFlash(__('Seo status code deleted', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Seo status code was not deleted', true));
		$this->redirect(array('action' => 'index'));
	}
}
?>