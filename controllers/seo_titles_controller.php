...
class SeoTitlesController extends SeoAppController {
	...
	function admin_view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid seo title', true));
	...
	function admin_add() {
		if (!empty($this->data)) {
			...
			if ($this->SeoTitle->save($this->data)) {
				$this->Session->setFlash(__('The seo title has been saved', true));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The seo title could not be saved. Please, try again.', true));
			}
		}
	...
	function admin_delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for seo title', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->SeoTitle->delete($id)) {
			$this->Session->setFlash(__('Seo title deleted', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Seo title was not deleted', true));
		$this->redirect(array('action' => 'index'));
	}
}
?>